...
export async function sendFcmToTokens(
  ...
  const badge  = toAbsoluteUrl("/icon-192.png", origin);
  const link   = toAbsoluteUrl(payload.url ?? "/", origin);

  const message = {
    // لا نضع حقل notification في المستوى الأعلى لمنع FCM من عرضه تلقائياً
    // ونتركه للـ service worker ليعرضه بشكل كامل (icon + badge + vibrate...)
    data: {
      title: payload.title,
      body:  payload.body,
      url:   link,
    ...
    webpush: {
      ...
      // notification هنا للـ webpush فقط — يضمن عرض الإشعار حتى لو الـ SW لم ينشّط
      notification: {
        title,
        body:               payload.body,
        icon,
