...
export async function awardReferralOnSignup(
  ...
  try {
    ...
    const wholeBonus = Math.round(bonus);

    // مكافأة الإحالة تذهب لرصيد الإحالة (cashback_balance) وليس الرصيد الرئيسي
    await adjustBalance(referrerId, "cashbackBalance", wholeBonus);
