...
export async function adjustBalance(
  ...
  const rows = await db.execute(
    sql.raw(
      `UPDATE users SET ${column} = (${column}::numeric + ${wholeDelta}.00)::numeric(12,2) WHERE id = ${userId} RETURNING ${column}::text AS new_balance`,
    ),
  );
...
export async function safeDebit(
  ...
  const rows = await db.execute(
    sql.raw(
      `UPDATE users SET ${column} = (${column}::numeric - ${wholeAmount}.00)::numeric(12,2) WHERE id = ${userId} AND ${column}::numeric >= ${wholeAmount}.00 RETURNING ${column}::text AS new_balance`,
    ),
  );
  const list = (rows as any).rows ?? rows;
