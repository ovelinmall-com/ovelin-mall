...
import { hashPassword, OWNER_EMAIL } from "./auth";
...
import { startSmmPriceSyncWorker } from "./services/smmPriceSyncWorker";
import { startDbCleanupWorker } from "./services/dbCleanupWorker";
import { initWhatsApp } from "../services/whatsapp-gateway.js";
import { warmUpCallVerify } from "./callVerifyService";

// ============================================================
// WARNING — The ONLY database for this project is Aiven PostgreSQL.
...
// ============================================================
// تحذير — قاعدة البيانات الوحيدة لهذا المشروع هي Aiven PostgreSQL.
...
const AUTHORISED_DB_HOST = "pg-3d55c6ad-dghmoaz-3d26.k.aivencloud.com";

export async function applyAdminPasswordFromEnv(): Promise<void> {
  const envPwd = process.env.ADMIN_PASSWORD;
  const effectivePwd = envPwd || DEFAULT_ADMIN_PASSWORD;
  try {
    // ابحث عن المالك بالـ email أولاً (requireAdmin يتحقق من email)
    const rows = await db
      .select()
      .from(usersTable)
    ...
    if (!admin) {
      // لو مافي مستخدم بهذا الـ email، أنشئ واحد جديد
      ...
      logger.info("تم إنشاء حساب الأدمن المرتبط بالـ email المعتمد");
    } else {
      // لا تُعِد تعيين كلمة السر إلا إذا حُدِّدت صراحةً عبر ADMIN_PASSWORD
      if (envPwd) {
        await db
          .update(usersTable)
          .set({ passwordHash: newHash })
          .where(eq(usersTable.id, admin.id));
        logger.info(`✅ حساب الأدمن جاهز — كلمة السر حُدِّثت من ADMIN_PASSWORD`);
      } else {
        logger.info(`✅ حساب الأدمن جاهز — username: ${admin.username}`);
      }
    }
  } catch (err) {
...
export function logNeonDatabaseConfirmation(): void {
  ...
  logger.info(
    `✅ قاعدة بيانات Aiven PostgreSQL — متصلة ومفعّلة\n` +
...
export async function runStartupTasks(): Promise<void> {
  ...
  try {
    await applyAdminPasswordFromEnv();
  } catch (err) {
    logger.warn({ err }, "فشل إنشاء/تحديث حساب الأدمن");
  }
  try {
  ...
  startSmmPriceSyncWorker(10_000); // مزامنة أسعار المورد كل 10 ثوانٍ
  // تهيئة WhatsApp Gateway — غير محجوب للـ startup
