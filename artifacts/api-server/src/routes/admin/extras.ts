...
import { sendOnesignalToAll } from "../../lib/services/onesignalService";
...
router.post("/admin/notify-user", requireAdmin, async (req, res) => {
  try {
    ...
    if (userId) {
      // ── رسالة موجهة لمستخدم واحد ──────────────────────────────
      notifyAndPush(userId, "admin", title, bodyText, link, "admin-broadcast");
      res.json({ sent: 1 });
    } else {
      // ── Broadcast لكل المستخدمين ────────────────────────────────
      // 1) OneSignal broadcast فوري لكل مشتركي الـ APK (استدعاء واحد فقط)
      sendOnesignalToAll({ title, body: bodyText, url: link, tag: "admin-broadcast" }).catch(() => {});

      // 2) أرسل للأدمن عبر OneSignal/FCM/VAPID كذلك
      sendPushToAdmin({ title, body: bodyText, url: link, tag: "admin-broadcast" }).catch(() => {});

      // 3) إشعار داخلي (in-app) + FCM/VAPID لكل مستخدم
