...
router.post("/admin/prize-draws/:id/draw-winner", requireAdmin, async (req, res) => {
  try {
    ...
    const updated = await db.update(prizeDrawsTable)
      ...
      .returning();
    await notify(winner.userId, "prize", "🏆 مبروك! فزت بالسحب", `لقد فزت بجائزة السحب #${id}`, "/prizes");
...
router.post("/admin/achievements/:id/grant/:userId", requireAdmin, async (req, res) => {
  try {
    ...
    if (ach[0]) {
      await notify(userId, "achievement", `🏆 فتحت إنجاز جديد: ${ach[0].name}`, ach[0].description, "/achievements");
    }
    res.json({ success: true });
  } catch (err: any) { res.status(500).json({ error: err?.message ?? "فشل" }); }
});
...
router.post("/admin/gift-cards", requireAdmin, async (req, res) => {
  try {
    ...
    if (recipientUserId) {
      await notify(Number(recipientUserId), "gift", "🎁 حصلت على بطاقة هدية!", `قيمتها ${amount} ج.س — كود: ${code}`, "/wallet");
    }
    res.json(inserted[0]);
  } catch (err: any) { res.status(500).json({ error: err?.message ?? "فشل" }); }
});
...
router.post("/admin/notify-user", requireAdmin, async (req, res) => {
  try {
    ...
    if (userId) {
      await notify(userId, "admin", title, bodyText, url ?? "/");
      res.json({ sent: 1 });
    } else {
      ...
      for (const u of users) {
        try {
          await notify(u.id, "admin", title, bodyText, url ?? "/");
          sent++;
        } catch { /* skip failed */ }
      }
      res.json({ sent });
