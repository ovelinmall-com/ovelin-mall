...
router.patch("/admin/orders/:id", requireAdmin, async (req, res) => {
  try {
    ...
    if (status && status !== order.status) {
      const isRejection = status === "rejected" || status === "failed";
      const isCancellation = status === "cancelled";
      const wasAlreadyTerminal = ["cancelled", "rejected", "failed"].includes(order.status);

      // ── رسالة حدث الطلب ─────────────────────────────────────
      const eventMsg: Record<string, string> = {
        processing: "بدأ تنفيذ الطلب",
        completed:  "تم الشحن بنجاح",
        delivered:  "تم التسليم",
        cancelled:  "تم إلغاء الطلب واسترداد المبلغ",
        rejected:   "الخدمة غير متوفرة — تم استرداد المبلغ لمحفظتك",
        failed:     "الخدمة غير متوفرة — تم استرداد المبلغ لمحفظتك",
      };
      await db.insert(orderEventsTable).values({
        orderId: id,
        status,
        message: eventMsg[status] ?? "تحديث الحالة بواسطة الإدارة",
      });

      // ── استرداد المبلغ فوراً عند الرفض/الإلغاء ──────────────
      let refunded = 0;
      if ((isRejection || isCancellation) && !wasAlreadyTerminal) {
        const refund = Number(order.finalPrice);
        const cb    = Number(order.cashbackUsed ?? 0);
        if (refund > 0) { await adjustBalance(order.userId, "balance", refund); refunded = refund; }
        if (cb    > 0)    await adjustBalance(order.userId, "cashbackBalance", cb);
      }

      // ── إشعار داخلي + Push للزبون ────────────────────────────
      if (isRejection && !wasAlreadyTerminal) {
        // رسالة رفض مخصصة مع المبلغ المسترد
        const refundMsg = refunded > 0
          ? `تم استرداد ${refunded.toFixed(0)} ج.س لمحفظتك فوراً`
          : "سيتم مراجعة طلبك";
        notifyAndPush(
          order.userId,
          "order",
          "⚠️ الخدمة غير متوفرة حالياً",
          `عذراً، لم نتمكن من إتمام شحن "${order.productName}" — ${refundMsg}`,
          "/orders",
          "order_rejected",
        );
      } else if (isCancellation && !wasAlreadyTerminal) {
        const refundMsg = refunded > 0
          ? `تم استرداد ${refunded.toFixed(0)} ج.س لمحفظتك`
          : "";
        notifyAndPush(
          order.userId,
          "order",
          "🚫 تم إلغاء الطلب",
          `الطلب #${order.id} — ${order.productName}${refundMsg ? " — " + refundMsg : ""}`,
          "/orders",
          "order_cancelled",
        );
      } else {
