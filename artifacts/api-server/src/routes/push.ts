...
router.post("/admin/push-live-test", requireUser, async (req, res) => {
  try {
    ...
    // ابحث عن الأدمن بنفس الطريقة التي تستخدمها sendPushToAdmin
    const adminRows = await db
      .select({ id: ut.id, username: ut.username })
      .from(ut)
      .where(or(eqFn(ut.email, "skandarabdoalatif@gmail.com"), eqFn(ut.username, "skandar"), eqFn(ut.username, "admin")))
      .limit(1);
