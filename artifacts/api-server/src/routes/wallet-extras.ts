...
router.get("/wallet/insights", requireUser, async (req, res) => {
  try {
    ...
    const fmtSDG = (n: number) => `${Math.floor(n).toLocaleString("en-US")},00 ج.س`;
    if (cashback >= 5) tips.push(`💰 لديك ${fmtSDG(cashback)} كاش باك جاهز للاستخدام في طلبك القادم.`);
    if (totalSpent30 > 100) tips.push(`📈 إنفاقك خلال 30 يوم: ${fmtSDG(totalSpent30)} — أنت زبون VIP نشط!`);
    ...
    if (cats.length > 0) {
      ...
      tips.push(`🎯 الفئة الأكثر إنفاقاً: ${top.category} (${fmtSDG(Number(top.total))}).`);
    }
      ...
      try {
        ...
        const r = await Promise.race([
          getAiClient().chat.completions.create({
            ...
            messages: [
              { role: "system", content: "أنت محلل مالي ذكي لمحفظة OVELIN. أنشئ من 3 إلى 5 نصائح قصيرة بالعربية الفصحى، كل نصيحة سطر واحد، بدون ترقيم. أضف الإيموجي المناسب في بداية كل نصيحة. كن دقيقاً ومحدّداً بالأرقام. اعرض جميع المبالغ بالجنيه السوداني بتنسيق: عدد صحيح بدون كسور متبوع بـ (,00 ج.س) مثل: 3,045,00 ج.س — لا تستخدم علامة الدولار $ أبداً." },
