...
/* ✅ تفعيل SW فوراً وتحكّمه في الصفحات بدون انتظار إعادة تحميل */
...
messaging.onBackgroundMessage(async (payload) => {
  ...
  /* ── أيقونة التطبيق — مطلقة دائماً ── */
  const icon      = toAbsoluteUrl("/icon-512.png");
  const badgeIcon = toAbsoluteUrl("/icon-badge.png");
  ...
  /* ──────────────────────────────────────────────────────────
     تصميم الإشعار الاحترافي
     • icon      → أيقونة مدورة كبيرة (يمين الإشعار)
     • badge     → أيقونة صغيرة بيضاء في شريط الحالة
     • image     → صورة موسّعة في الأسفل عند الفرد (اختياري)
     • vibrate   → اهتزاز مزدوج للتنبيه
     • silent    → false  → يصدر صوتاً
     • dir       → rtl    → محاذاة عربية صحيحة
  ────────────────────────────────────────────────────────── */
...
/* ---- VAPID Web Push (احتياطي — غير FCM) ----
   Firebase messaging SDK يتعامل مع رسائل FCM فقط.
   هذا الـ handler يعالج إشعارات VAPID الخام التي ترسلها web-push library.
   نتحقق من أن الـ payload ليس FCM (FCM مشفّر ولا يمكن قراءته كـ JSON) ---- */
self.addEventListener("push", (event) => {
  if (!event.data) return;
  let payload;
  try { payload = event.data.json(); } catch { return; /* ليس JSON — FCM مشفّر أو غير مدعوم */ }

  // إذا كان FCM (Firebase messaging SDK يتعامل معه) — لا نتدخل
  if (!payload || (!payload.title && !payload.body)) return;

  const icon      = toAbsoluteUrl("/icon-512.png");
  const badgeIcon = toAbsoluteUrl("/icon-badge.png");
  const title     = payload.title ?? "Ovelin Mall";
  const body      = payload.body  ?? "";
  const url       = payload.url   ?? "/";
  const tag       = payload.tag   ?? "ovelin";
