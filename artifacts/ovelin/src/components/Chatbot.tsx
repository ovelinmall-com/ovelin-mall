...
export function Chatbot() {
  ...
  return (
    <>
      {/* === Floating launcher: pink ring + transparent center + OVELIN AI label === */}
      <div className="fixed bottom-20 left-4 z-50 flex flex-col items-center gap-1.5 select-none">
        <button
          onClick={() => setOpen(!open)}
          aria-label="OVELIN AI Assistant"
          className="relative w-16 h-16 rounded-full transition-all duration-300 hover:scale-110 active:scale-95 group"
        >
          {/* Soft outer halo */}
          <span className="absolute -inset-2 rounded-full bg-pink-400/30 blur-xl group-hover:bg-pink-400/50 transition" />
          {/* Animated ping ring */}
          <span className="absolute inset-0 rounded-full ring-2 ring-pink-400/70 animate-ping" />
          {/* Solid pink ring with transparent center */}
        ...
        <div
          className="px-2.5 py-0.5 rounded-full bg-white/95 dark:bg-zinc-900/95 text-[10px] font-extrabold text-pink-600 shadow-md shadow-pink-500/20 border border-pink-200 dark:border-pink-900 backdrop-blur-sm whitespace-nowrap tracking-wider"
          dir="ltr"
        >
          OVELIN AI
        </div>
