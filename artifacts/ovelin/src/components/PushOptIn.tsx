...
// ── تسجيل External User ID في OneSignal ──────────────────────────────────

/**
 * يربط userId بجهاز OneSignal عبر external_user_id.
 * هذا يتيح إرسال الإشعارات لاحقاً عبر userId بدون الحاجة لـ player_id.
 * يجب استدعاؤه بعد كل تسجيل دخول ناجح.
 */
export async function registerExternalUserId(userId: number): Promise<void> {
  try {
    // تسجيل في السيرفر لحفظ external_id في DB
    await fetch(`${BASE}api/push/register-external-id`, {
      method: "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ externalUserId: String(userId) }),
    }).catch(() => {});

    // إعلام OneSignal bridge بالـ external_id (يربط الجهاز بالمستخدم)
    const gn = (window as any).gonative || (window as any).median;
    if (!gn) return;

    // OneSignal SDK v5: login(externalId)
    if (typeof gn?.onesignal?.login === "function") {
      console.info(LOG, `📡 gonative.onesignal.login(${userId}) — SDK v5`);
      gn.onesignal.login(String(userId));
    }
    // OneSignal SDK v3/v4: setExternalUserId
    if (typeof gn?.onesignal?.setExternalUserId === "function") {
      console.info(LOG, `📡 gonative.onesignal.setExternalUserId(${userId}) — SDK v3/v4`);
      gn.onesignal.setExternalUserId(String(userId));
    }
    // Median native push tag (لتصفية الإشعارات)
    if (typeof gn?.push?.tags?.setTags === "function") {
      gn.push.tags.setTags({ userId: String(userId), isAdmin: "false" });
    }
  } catch (err: any) {
    console.warn(LOG, "⚠️ registerExternalUserId error:", err?.message);
  }
}
...
export function PushOptIn() {
  ...
  useEffect(() => {
    ...
    // Step 1b: ربط userId بـ OneSignal عبر external_user_id (حل مشكلة player_id = 0)
    registerExternalUserId(user.id).catch(() => {});
    ...
    // If APK, auto-remove any existing VAPID subscriptions so browser doesn't duplicate
    const webPushTimer = setTimeout(() => {
      if (!isMedianApp()) {
        ...
        console.info(LOG, "📱 Median APK mode — removing VAPID subscriptions from server…");
        // حذف كل اشتراكات VAPID من DB (الكروم يستخدمها — لا نريدها في APK)
        fetch(`${BASE}api/push/vapid-unsubscribe`, {
          method: "DELETE",
          credentials: "include",
        })
          .then((r) => console.info(LOG, `✅ VAPID unsubscribe: HTTP ${r.status}`))
          .catch((e) => console.warn(LOG, "⚠️ VAPID unsubscribe failed:", e?.message));
        // إلغاء اشتراك SW محلياً أيضاً إن وُجد
