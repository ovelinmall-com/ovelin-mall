  ...
  return (
    <div className="min-h-[100dvh] pb-20">
      ...
      <div className="mx-4 bg-white dark:bg-zinc-900 rounded-2xl p-3 mb-3 space-y-2">
        <input value={form.key} onChange={(e) => setForm({ ...form, key: e.target.value })} placeholder="مفتاح الاختبار (مثل: hero_cta)" className="w-full px-3 py-2 rounded-lg bg-white dark:bg-zinc-800 text-sm" />
        <input value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="الوصف" className="w-full px-3 py-2 rounded-lg bg-white dark:bg-zinc-800 text-sm" />
        <input value={form.variants} onChange={(e) => setForm({ ...form, variants: e.target.value })} placeholder="المتغيرات (مفصولة بفاصلة)" className="w-full px-3 py-2 rounded-lg bg-white dark:bg-zinc-800 text-sm" />
                ...
                {t.breakdown.map((b: any) => {
                  ...
                  return (
                    <div key={b.variant} className="text-xs flex items-center gap-2">
                      ...
                      <div className="flex-1 h-4 bg-white dark:bg-zinc-800 rounded-full overflow-hidden">
