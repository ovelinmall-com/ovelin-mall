  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-4 gap-2">
        <ToolCard icon={<BarChart2 className="w-4 h-4" />} label="إحصائيات" active={showAnalytics} onClick={() => setShowAnalytics(!showAnalytics)} accent="from-rose-500 to-pink-600" />
  ...
  return (
    <div className="fancy-card rounded-3xl p-3 space-y-2">
      <div className="flex gap-1.5 flex-wrap items-center">
        ...
        <button onClick={() => setFilters((f: any) => ({ ...f, status: "" }))} className={cn("px-2.5 py-1 rounded-lg text-[10px] font-bold", !filters.status ? "bg-pink-600 text-white" : "bg-pink-50 text-pink-700")}>الكل</button>
        {STATUSES.map((s) => (
          <button key={s} onClick={() => setFilters((f: any) => ({ ...f, status: s }))} className={cn("px-2.5 py-1 rounded-lg text-[10px] font-bold", filters.status === s ? "bg-pink-600 text-white" : "bg-pink-50 text-pink-700")}>
            {STATUS_LABEL[s]}
          </button>
        ))}
      </div>
      <div className="flex gap-1.5 flex-wrap items-center">
        ...
        <button onClick={() => setFilters((f: any) => ({ ...f, priority: "" }))} className={cn("px-2.5 py-1 rounded-lg text-[10px] font-bold", !filters.priority ? "bg-pink-600 text-white" : "bg-rose-50 text-rose-700")}>الكل</button>
        {PRIORITIES.map((p) => (
          <button key={p} onClick={() => setFilters((f: any) => ({ ...f, priority: p }))} className={cn("px-2.5 py-1 rounded-lg text-[10px] font-bold", filters.priority === p ? "bg-rose-600 text-white" : PRIORITY_COLOR[p] ?? "bg-rose-50 text-rose-700")}>
            {PRIORITY_LABEL[p]}
          </button>
        ))}
      </div>
      <div className="flex gap-1.5 flex-wrap items-center">
        ...
        <select
          ...
          className="bg-pink-50 rounded-lg px-2 py-1 text-[10px] font-bold text-pink-700 focus:outline-none"
        >
        ...
        <input
          ...
          className="bg-pink-50 rounded-lg px-2 py-1 text-[10px] font-bold text-pink-700 focus:outline-none w-24"
        />
      </div>
      <div className="flex gap-1.5">
        <input
          ...
          className="flex-1 bg-pink-50 rounded-xl px-3 py-2 text-[11px] focus:outline-none"
        />
    ...
    <div onClick={onOpen} className="fancy-card rounded-3xl p-3 cursor-pointer active:scale-[0.99]">
      ...
      <div className="mt-2 flex items-center gap-1.5 flex-wrap">
        ...
        {(t.tags ?? []).slice(0, 3).map((tag: string) => (
          <span key={tag} className="text-[10px] font-bold rounded-full bg-pink-50 text-pink-700 px-2 py-0.5">#{tag}</span>
        ))}
...
function TicketView({ id, agents, onBack }: { id: number; agents: Agent[]; onBack: () => void }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-3xl bg-gradient-to-br from-pink-50 to-rose-50 p-3 border border-pink-200 shadow-sm">
        <div className="flex items-start justify-between gap-2">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          <div>
            ...
            <div className="flex flex-wrap gap-1">
              {STATUSES.map((s) => (
                <button key={s} disabled={busy === `status-${s}`} onClick={() => setStatus(s)} className={cn("text-[10px] font-bold px-2 py-1 rounded-lg disabled:opacity-50", ticket.status === s ? "bg-pink-600 text-white" : "bg-pink-50 text-pink-700")}>
                  {STATUS_LABEL[s]}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1">
              {PRIORITIES.map((p) => (
                <button key={p} disabled={busy === `prio-${p}`} onClick={() => setPriority(p)} className={cn("text-[10px] font-bold px-2 py-1 rounded-lg disabled:opacity-50", ticket.priority === p ? "bg-rose-600 text-white" : "bg-rose-50 text-rose-700")}>
                  {PRIORITY_LABEL[p]}
                </button>
              ))}
            </div>
      ...
      {/* AI Summary */}
      <div className="rounded-3xl bg-gradient-to-br from-rose-50 to-rose-50 p-3 border border-rose-200">
      ...
      {/* Messages */}
      <div className="rounded-3xl bg-gradient-to-b from-pink-50/50 to-white p-3 border border-pink-100 space-y-2 max-h-[55vh] overflow-y-auto">
        {messages.map((m: any) => {
      ...
      {ticket.status === "closed" ? (
        ...
        <div className="fancy-card rounded-3xl p-2 space-y-2">
          ...
          {showTemplatePicker && templates.length > 0 && (
            <div className="border border-pink-100 rounded-2xl p-2 max-h-40 overflow-y-auto bg-pink-50/40 space-y-1">
              {templates.map((t) => (
                <button
                  key={t.id}
                  onClick={() => { setBody(t.body); setShowTemplatePicker(false); }}
                  className="fancy-card w-full text-right rounded-xl p-2 hover:bg-pink-50"
                >
          ...
          <div className="flex items-end gap-1.5">
            ...
            <button onClick={() => fileRef.current?.click()} disabled={busy === "attach"} className="p-2.5 rounded-2xl bg-pink-50 text-pink-600 disabled:opacity-50">
            ...
            <textarea
              ...
              className="flex-1 resize-none bg-pink-50/40 rounded-2xl px-3 py-2 text-[12.5px] focus:outline-none"
            />
...
function AnalyticsPanel() {
  ...
  return (
    <div className="rounded-3xl bg-gradient-to-br from-rose-50 to-rose-50 border border-rose-200 p-3 space-y-3">
...
function FaqAnalyticsPanel() {
  ...
  if (err) {
    return (
      <div className="rounded-3xl bg-pink-50 p-4 border border-pink-200 text-sm text-pink-700">
        تعذر التحميل: {err}
      </div>
    );
  }
