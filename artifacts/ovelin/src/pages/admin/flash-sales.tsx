  ...
  return (
    <div className="min-h-[100dvh] pb-20">
      ...
      <div className="mx-4 bg-white dark:bg-zinc-900 rounded-2xl p-3 mb-3 space-y-2">
        <select value={productId} onChange={(e) => setProductId(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-white dark:bg-zinc-800 text-sm">
          <option value="">اختر منتج</option>
          {products.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
        </select>
        <div className="flex gap-2">
          <input value={discountPct} onChange={(e) => setDiscount(e.target.value)} type="number" placeholder="الخصم %" className="flex-1 px-3 py-2 rounded-lg bg-white dark:bg-zinc-800 text-sm" />
          <input value={duration} onChange={(e) => setDuration(e.target.value)} type="number" placeholder="المدة (د)" className="flex-1 px-3 py-2 rounded-lg bg-white dark:bg-zinc-800 text-sm" />
        </div>
