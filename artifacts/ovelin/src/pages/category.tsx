...
export default function CategoryPage() {
  ...
  const { data: freshProducts, isLoading } = useListProducts({
    category: slug,
    ...(showPlatformTabs && platform ? { platform } : {}),
  });

  // الاحتفاظ بالمنتجات السابقة — تبقى مرئية أثناء تحميل الفلتر الجديد
  const stableProductsRef = useRef(freshProducts);
  if (freshProducts !== undefined) stableProductsRef.current = freshProducts;
  const products = stableProductsRef.current;
                ...
                <div className="fancy-card rounded-3xl active:scale-[0.98] transition overflow-hidden">
                  ...
                  <div
                    className={cn(
                      ...
                      pmeta
                        ? `${pmeta.bgColor}`
                        : "bg-gradient-to-l from-white via-white to-pink-100",
                    )}
                  >
                    <div
                      className={cn(
                  ...
                  <div className="px-4 py-3 flex items-center justify-between border-t border-pink-50">
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {formatSDG(p.price)} ج.س
                      </div>
                      {p.oldPrice && (
                        <div className="text-[11px] text-muted-foreground line-through">
                          {formatSDG(p.oldPrice)} ج.س
                        </div>
                      )}
                      {p.oldPrice && (
