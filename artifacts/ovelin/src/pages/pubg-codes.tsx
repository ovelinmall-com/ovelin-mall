...
export default function PubgCodes() {
  ...
  const { data: dashboard } = useGetMyDashboard({ query: { queryKey: getGetMyDashboardQueryKey(), enabled: !!me, refetchInterval: 60_000 } });
  // ✅ رصيد حي يتحدث فوراً عبر WebSocket
            ...
            >
              <div className="rounded-2xl border border-pink-500/30 p-4"
                ...
                <div className="h-2 rounded-full bg-white/10 overflow-hidden">
                  <motion.div
                    className={`h-2 rounded-full ${canAfford ? "bg-gradient-to-r from-emerald-500 to-green-400" : "bg-gradient-to-r from-red-500 to-pink-500"}`}
                    initial={{ width: 0 }}
