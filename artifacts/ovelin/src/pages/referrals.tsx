  ...
  return (
    <AppLayout>
      ...
      <div className="px-5 space-y-4 pb-4">
        {/* تنبيه إيقاف الإحالة */}
        {(ref as any)?.referralEnabled === false && (
          <div className="flex items-start gap-3 rounded-2xl bg-red-500 px-4 py-3 text-white shadow-lg">
            <AlertTriangle className="w-5 h-5 shrink-0 mt-0.5" />
            <div>
              <div className="text-sm font-extrabold">نظام الإحالة موقوف حالياً</div>
              <div className="text-xs opacity-90 mt-0.5">لن تحصل على أي مكافآت ريثما يُعاد تفعيله من الإدارة</div>
            </div>
          </div>
        )}
        ...
        >
          ...
          <div className="relative flex flex-wrap items-center gap-2 mb-2">
            ...
            {signupBonusAmt > 0 && (ref as any)?.referralEnabled !== false && (
        ...
        <div className="fancy-card rounded-3xl p-4">
          ...
          <div className="space-y-2">
            {earnings?.recent.map((c) => (
              <div
                key={c.id}
                className="flex items-center justify-between rounded-2xl bg-gradient-to-l from-white/70 to-transparent p-3"
              >
                <div className="flex items-center gap-2 min-w-0">
                  <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-pink-400 to-pink-500 text-white font-black uppercase flex items-center justify-center text-sm shrink-0">
                    {c.fromUsername[0]}
                  </div>
        ...
        <div className="fancy-card rounded-3xl p-4">
          ...
          {ref?.referrals?.map((r, i) => (
            <div
              key={i}
              className="flex items-center justify-between rounded-2xl bg-white/40 p-3 mb-2 last:mb-0"
            >
              <div className="flex items-center gap-2">
                <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-pink-400 to-pink-500 text-white font-black uppercase flex items-center justify-center text-sm">
                  {r.username[0]}
                </div>
                <div>
        ...
        <div className="fancy-card rounded-3xl p-4">
          ...
          {leaderboard?.map((row, i) => (
            <div
              key={i}
              className="flex items-center justify-between rounded-2xl p-3 mb-2 last:mb-0 bg-gradient-to-l from-white to-transparent"
            >
...
function ReferralWithdrawForm() {
  const [, setLocation] = useLocation();
  return (
    <div className="mt-3 rounded-2xl border border-pink-200 bg-white/40 p-3 text-xs text-pink-800">
