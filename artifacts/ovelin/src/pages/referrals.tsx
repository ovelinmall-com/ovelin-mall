...
const SHARE_TEMPLATES = [
  {
    ...
    href: (link: string) =>
      `https://wa.me/?text=${encodeURIComponent(`جرّب OVELIN — أفضل متجر خدمات رقمية. سجّل عبر رابطي وابدأ:\n${link}`)}`,
  },
  {
    name: "تيليجرام",
    color: "bg-sky-500",
    href: (link: string) =>
      `https://t.me/share/url?url=${encodeURIComponent(link)}&text=${encodeURIComponent("جرّب OVELIN — أفضل متجر خدمات رقمية")}`,
  },
  {
    name: "تويتر",
    color: "bg-zinc-900",
    href: (link: string) =>
      `https://twitter.com/intent/tweet?text=${encodeURIComponent(`جرّب OVELIN! ${link}`)}`,
  },
  {
    name: "فيسبوك",
    color: "bg-blue-600",
...
export default function ReferralsPage() {
  ...
  function share() {
    if (typeof navigator !== "undefined" && "share" in navigator) {
      navigator
        .share({ title: "OVELIN", text: "انضم إلى OVELIN", url: link })
        .catch(() => {});
    } else {
      copy();
    }
  ...
  return (
    <AppLayout>
      <PageHeader title="نظام الإحالات" subtitle="ادعُ أصدقاءك واربح 5% مدى الحياة" back="/account" />
      ...
      <div className="px-5 space-y-4 pb-4">
        ...
        >
          ...
          <div className="relative flex items-center gap-2 text-xs opacity-95">
            <Sparkles className="w-4 h-4" /> اربح 5% من كل عملية شراء يقوم بها صديقك للأبد
          </div>
          <h2 className="relative mt-1 text-2xl font-black leading-tight">
          ...
          >
            <Share2 className="w-4 h-4" /> مشاركة الدعوة
          </button>
        ...
        <div className="fancy-card rounded-3xl p-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="rounded-2xl bg-gradient-to-br from-pink-50 to-pink-100 p-3 text-center">
              ...
              <div className="text-lg font-black text-pink-900 mt-0.5">
                ${earnings?.thisMonth ?? "0.00"}
              </div>
            </div>
            <div className="rounded-2xl bg-gradient-to-br from-rose-50 to-rose-100 p-3 text-center">
              ...
              <div className="text-lg font-black text-rose-900 mt-0.5">
                ${earnings?.lastMonth ?? "0.00"}
              </div>
            </div>
            <div className="rounded-2xl bg-gradient-to-br from-pink-50 to-pink-100 p-3 text-center">
              <div className="text-[10px] text-pink-700">إجمالي</div>
              <div className="text-lg font-black text-pink-900 mt-0.5">
                ${earnings?.allTime ?? ref?.totalEarned ?? "0.00"}
              </div>
            </div>
          </div>
