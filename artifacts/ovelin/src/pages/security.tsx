...
export default function SecurityPage() {
  ...
  const changePwd = useMutation({
    ...
    onError: (e: any) => {
      setPwdErr(e?.data?.error ?? e?.message ?? "تعذر تغيير كلمة المرور");
    },
  });
  ...
  function submitPwd(e: React.FormEvent) {
    e.preventDefault();
    setPwdErr(null);
    if (newPwd.length < 4) return setPwdErr("كلمة المرور قصيرة (4 أحرف على الأقل)");
    if (newPwd !== conPwd) return setPwdErr("تأكيد كلمة المرور غير مطابق");
  ...
  return (
    <AppLayout>
      ...
      <div className="px-4 space-y-3 pb-4">
        ...
        {/* ── تغيير كلمة المرور ── */}
        <form onSubmit={submitPwd} className="fancy-card rounded-3xl p-4 space-y-3">
          ...
          <input
            ...
            className="w-full rounded-2xl border border-pink-200 bg-white/40 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-pink-400"
          />

          {/* رسالة خطأ */}
          {pwdErr && (
            <div className="rounded-xl bg-pink-50 border border-pink-200 text-pink-700 text-xs p-2.5 flex gap-2 items-start">
          ...
          {/* رسالة نجاح */}
          {pwdDone && (
            <div className="rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs p-2.5 flex gap-2 items-start">
          ...
          <button
            type="submit"
            disabled={changePwd.isPending || !curPwd || !newPwd || !conPwd}
            className="w-full rounded-2xl py-2.5 bg-gradient-to-r from-pink-500 to-rose-600 text-white text-sm font-bold shadow disabled:opacity-60 active:scale-95 transition"
          >
            {changePwd.isPending ? "جارٍ الحفظ..." : "حفظ كلمة المرور"}
          </button>
        </form>
        ...
        <div className="fancy-card rounded-3xl p-4">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 rounded-2xl border border-pink-200 bg-white/40 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-pink-400"
              dir="ltr"
            />
            <button
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <input
                ...
                type="number"
                placeholder="بدون"
                className="mt-1 w-full rounded-xl border border-pink-200 bg-white/40 px-3 py-2 text-sm"
              />
            </div>
            <div>
              <label className="text-[10px] text-pink-800 font-semibold">
