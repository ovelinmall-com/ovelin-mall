        ...
        <div className="px-4 space-y-3">
          ...
          {plans.map((p) => (
            <div key={p.id} className="fancy-card dark:bg-zinc-900 rounded-2xl p-4 dark:border-pink-900/30">
              ...
              <input value={target} onChange={(e) => setTarget(e.target.value)} placeholder="رابط/معرّف الهدف" className="w-full mt-3 px-3 py-2 rounded-lg bg-white dark:bg-zinc-800 text-sm" />
          ...
          {mySubs.map((s) => (
            <div key={s.id} className="fancy-card dark:bg-zinc-900 rounded-2xl p-4 dark:border-pink-900/30">
              ...
              <div className="flex gap-2 mt-3">
                <button onClick={() => toggleRenew(s.id)} className="flex-1 py-1.5 rounded-full bg-white text-pink-700 text-xs font-bold">{s.autoRenew ? "إيقاف التجديد" : "تفعيل التجديد"}</button>
