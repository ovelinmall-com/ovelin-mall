  ...
  return (
    <AppLayout>
          ...
          {!c?.totalCount && (
            <div className="mt-4 rounded-2xl bg-white/15 backdrop-blur p-3 text-[12px] flex items-center gap-2">
              ...
              <span className="opacity-90">
                فريق OVELIN MALL يرد خلال دقائق غالباً. أنشئ تذكرة جديدة لأي استفسار أو مشكلة.
              </span>
            </div>
          )}
        </div>
      ...
      <div className="px-4 -mt-5 pb-4 space-y-3">
        ...
        {/* ── Agent + WhatsApp Card ──────────────────────────── */}
        <div className="fancy-card rounded-3xl p-5 text-center overflow-hidden relative">
          {/* Subtle background glow */}
          <div className="absolute -top-8 left-1/2 -translate-x-1/2 w-48 h-48 bg-red-200/30 rounded-full blur-3xl pointer-events-none" />

          {/* Agent icon with rotating glow */}
          <div className="relative w-24 h-24 mx-auto mb-3">
            <motion.div
              className="absolute inset-0 rounded-full"
              style={{ background: "conic-gradient(from 0deg, transparent 0%, transparent 45%, rgba(220,38,38,0.85) 65%, rgba(252,165,165,0.5) 75%, transparent 88%)" }}
              animate={{ rotate: 360 }}
              transition={{ duration: 3.5, repeat: Infinity, ease: "linear" }}
            />
            <div className="absolute inset-[3px] rounded-full bg-gradient-to-br from-red-500 via-rose-600 to-red-700 flex items-center justify-center shadow-[0_0_24px_rgba(220,38,38,0.55)]">
          ...
          <div className="flex items-center justify-center gap-1.5 mb-1">
            ...
            <span className="text-xs font-black text-emerald-600">متاح الآن</span>
          </div>
          <div className="text-sm font-black text-pink-900 mb-4">
            متوفر على مدار اليوم 24 ساعة
          </div>

          {/* WhatsApp button */}
          <a
            href="https://wa.me/249902288269"
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center justify-center gap-2.5 w-full py-3.5 rounded-2xl bg-gradient-to-l from-[#25D366] to-[#128C7E] text-white font-black text-sm shadow-[0_6px_20px_rgba(37,211,102,0.4)] active:scale-95 transition"
          >
            <FaWhatsapp className="w-5 h-5 text-white" />
            اضغط هنا لمراسلة الدعم عبر الواتساب
          </a>
        </div>
        ...
        <div className="fancy-card rounded-3xl p-3 space-y-2">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-2xl border border-pink-200 bg-white/40 px-4 py-2.5 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-pink-400"
            />
          </div>
        ...
        {(!filtered || filtered.length === 0) && (
          <div className="fancy-card rounded-3xl p-8 text-center">
            <div className="mx-auto w-16 h-16 rounded-full bg-white flex items-center justify-center text-pink-500 mb-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[11px] text-muted-foreground mt-0.5 flex items-center gap-1.5 flex-wrap">
                    ...
                    {t.category && (
                      <>
                        <span>•</span>
                        <span className="rounded-md bg-white text-pink-700 px-1.5 py-0.5 text-[9px] font-bold">
                          {t.category}
                        </span>
                      </>
                    )}
